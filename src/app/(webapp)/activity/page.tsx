...
import ActivityCard, { Transaction } from "../../../features/activity/ActivityCard";
...
function ActivityPageContent() {
	...
	const filteredTransactions = useMemo(() => {
		...
		return mockTransactions.filter((tx) => tx.type === activeFilter);
	}, [activeFilter]);

	// Separate Today's and Yesterday's transactions
	...
	if (selectedTx) {
		const isArrowBanner = selectedTx.type === "withdrawal" || selectedTx.type === "payout";
					...
					<div className="bg-white border border-neutral-light rounded-[20px] p-5 shadow-sm">
						<div className="flex items-center justify-between">
							...
							<div className="flex items-center gap-3 text-neutral-light-active">
								<button type="button" className="hover:text-primary transition-colors cursor-pointer bg-transparent border-0">
							...
							{isArrowBanner ? (
								<>
									...
									<div className="flex flex-col z-10">
										<span className="text-3xl font-extrabold">{selectedTx.amount}</span>
								...
								<>
									...
									<div className="flex flex-col items-center justify-center flex-1 z-10 text-center">
										<span className="text-3xl font-extrabold text-primary-darker">{selectedTx.amount}</span>
						...
						<div className="mt-8 space-y-4">
							...
							<div className="flex justify-between text-sm py-1">
								...
								<span className="font-semibold text-neutral-dark">{selectedTx.groupName}</span>
							</div>
							...
							<div className="flex justify-between text-sm py-1">
								...
								<span className="font-semibold text-neutral-dark flex items-center gap-1.5 truncate max-w-37.5 sm:max-w-none">
									<span>{selectedTx.txNumber}</span>
									<button
										type="button"
										onClick={() => handleCopyTx(selectedTx.txNumber)}
							...
							<div className="flex justify-between text-sm py-1">
								...
								<span className="font-semibold text-neutral-dark">{selectedTx.date}</span>
							</div>
						</div>
			...
			<div className="space-y-6">
					...
					{(["all", "contribution", "payout", "withdrawal"] as const).map((filter) => {
						...
						return (
							<button
								...
								className={`whitespace-nowrap rounded-full px-5 py-2 text-xs font-semibold cursor-pointer border-0 shadow-sm transition-colors ${
									active
						...
						<div className="space-y-3.5">
							...
							<div className="space-y-3.5">
								{groupedTransactions.today.map((tx) => (
									<ActivityCard key={tx.id} tx={tx} onViewDetails={setSelectedTx} />
								))}
							</div>
						</div>
					)}
				...
				{filteredTransactions.length > 0 && (
					<div className="flex justify-end mt-6 pt-2">
						<button
							type="button"
							className="bg-primary-darker hover:bg-primary-dark-active text-white text-xs font-semibold px-6 py-2.5 rounded-full transition-colors cursor-pointer border-0 shadow-sm"
						>
							Show more
						</button>
					</div>
